                        ...
                        <div class="container mx-auto p-4">
                            ...
                            <form action="{{ route('admin.reservationList.update', $appointment->id) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <x-text-input id="reservation_datetime" class="block mt-1 w-full" type="text"
                                        name="name" :value="old(
                                ...
                                <div class="mb-4">
                                    ...
                                    <x-select id="vehicle_type" class="block mt-1 w-full" name="vehicle_type">
                                        <option value=""
                                            {{ is_null($appointment->vehicle_type) ? 'selected' : '' }}>
                                            選択してください
                                        </option>
                                        <option value="sedan"
                                            {{ $appointment->vehicle_type == 'sedan' ? 'selected' : '' }}>セダン</option>
                                        <option value="suv"
                                            {{ $appointment->vehicle_type == 'suv' ? 'selected' : '' }}>SUV</option>
                                        <option value="wagon"
                                            {{ $appointment->vehicle_type == 'wagon' ? 'selected' : '' }}>ワゴン</option>
                                        <option value="track"
                                            {{ $appointment->vehicle_type == 'track' ? 'selected' : '' }}>トラック</option>
                                        <option value="other"
                                            {{ $appointment->vehicle_type == 'other' ? 'selected' : '' }}>その他</option>
                                    </x-select>
                                ...
                                <div class="mt-4">
                                    ...
                                    <x-checkbox name="additional_services[]" value="エンジンオイル交換" label="エンジンオイル交換"
                                        :checked="old('additional_services', false)" :disabled="false" class="mt-2" />
                                    <x-checkbox name="additional_services[]" value="タイヤローテション[前⇔後]"
                                        label="タイヤローテション[前⇔後]" :checked="old('additional_services', false)" :disabled="false" class="mt-2" />
                                    <x-checkbox name="additional_services[]" value="タイヤ付替[夏⇔冬シーズンチェンジ"
                                        label="タイヤ付替[夏⇔冬シーズンチェンジ" :checked="old('additional_services', false)" :disabled="false"
                                        class="mt-2" />
                                </div>
                                <!-- 更新ボタン -->
