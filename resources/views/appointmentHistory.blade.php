                ...
                <div class="overflow-x-auto mt-4">
                                ...
                                @foreach ($filteredGroupedAppointments as $appointNumber => $appointmentsGroup)
                                    ...
                                    @if ($appointmentsGroup->count() > 0)
                                        <thead class="sticky top-0 z-10">
                                            ...
                                            <tr>
                                                <th class="px-4 py-2 text-left bg-gray-200 " style="width: 10%;">予約番号
                                                </th>
                                                <th class="px-4 py-2 text-left bg-gray-200 " style="width: 8%;">
                                                    台数</th>
                                                <th class="px-4 py-2 text-left bg-gray-200 " style="width: 20%;">予約日時
                                                </th>
                                            </tr>
                                        </thead>
                    ...
                    <form method="GET" action="{{ route('appointmentList.index') }}" class="mb-6">
                        <div class="flex items-center gap-4 mt-6">
                            <!-- 日付並び替え -->
                            <div>
                                <label for="sort_date" class="block text-sm font-medium text-gray-700">日付</label>
                                <select name="sort_date" id="sort_date" class="border-gray-300 rounded-md shadow-sm">
                                    <option value="desc" {{ request('sort_date') == 'desc' ? 'selected' : '' }}>新しい順
                                    </option>
                                    <option value="asc" {{ request('sort_date') == 'asc' ? 'selected' : '' }}>古い順
                                    </option>
                                </select>
                            </div>
                            ...
                            <!-- 予約番号フィルター -->
                            <div>
                                <label for="appoint_number" class="block text-sm font-medium text-gray-700">予約番号</label>
