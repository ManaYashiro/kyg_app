                ...
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-auto mt-4">
                        <div class="container mx-auto p-4">
                            ...
                            <form action="{{ route('admin.notificationSetting.store') }}" method="POST" enctype="multipart/form-data" id="notification-form">
                                @csrf
                                @method('POST')
                                    ...
                                    <div class="flex items-center">
                                        <label for="is_active_1" class="mr-4">
                                            <input type="radio" id="is_active_1" name="is_active" value="1"
                                                {{ old('is_active', $notification->is_active ?? '') == 1 ? 'checked' : '' }} required />
                                            {{ __('公開') }}
                                        </label>

                                        <label for="is_active_0">
                                            <input type="radio" id="is_active_0" name="is_active" value="0"
                                                {{ old('is_active', $notification->is_active ?? '') == 0 ? 'checked' : '' }} required />
                                            {{ __('非公開') }}
                                        </label>
                                    </div>
                            ...
                            <!-- 削除ボタン -->
                            @isset($notification)
                            <form action="{{ route('admin.notificationSetting.destroy', $notification->id) }}" method="POST" onsubmit="return confirm('本当に削除しますか？');">
                                @csrf
                                @method('DELETE')
                                <div class="mb-4 flex justify-end">
                                ...
                                <tbody>
                                    ...
                                    <tr class="clickable-row" data-id="{{ $notification->id }}"
                                        ...
                                        <td class="border px-4 py-2" style="font-size: 0.75rem;">
                                            ...
                                            @endphp
                                            @switch($category)
                                                @case(1) システムメンテナンス情報 @break
                                                @case(2) 新機能・サービスの追加 @break
                                                @case(3) キャンペーン・割引情報 @break
                                                @case(4) 予約状況や混雑情報 @break
                                                @case(5) 不具合・障害情報 @break
                                                @case(6) 特別イベント・ワークショップの案内 @break
                                                @case(7) お客様の声や事例紹介 @break
                                                @case('no_preference') 指定なし @break
                                                @default 未設定 @break
                                            @endswitch
                                        </td>
                            ...
                            <div class="mt-4 mb-4 flex justify-end">
                                <button id="delete-btn" class="ms-3 bg-red-500 inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 focus:bg-red-800 active:bg-red-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" disabled>削除</button>
                            </div>
                        </div>
                    </div>
                </div>
